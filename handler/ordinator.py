...
class Ordinator():
    ...
    def freeup(self, vram):
        ...
        remain_vram = get_remain_vram()
        desired_vram = vram * (1+MEMORY_REDUNDANCY)
        print(f"(Ordinator) {remain_vram} remaining while {desired_vram} should be freed up.")
        ...
        if len(cuda_idlers) == 0:
            ...
            disp.yellow(f"Impossible to free up {vram_to_release}MB from CUDA.")
            return False

        kick_choices = decide_kick_idlers(cuda_idlers, vram_to_release, desired_vram, )
        if not kick_choices:
            disp.yellow(f"Unable to find a satisfiable combination of cuda idlers to kick for freeing up {vram_to_release}MB.")
    ...
    def force_reallocate(self, model_class, model_name):
        target_model_info = self.callback_factories[f'get_{model_class}_models_info']()([model_name])
        #print("(Ordinator) reallocate() target_model_info:",target_model_info)
        if target_model_info[model_name]['flex_device'] == 'cuda':
            return True
        if not target_model_info[model_name]['device'] == 'flex':
            disp.red("You should not config the device of an LLM to be cpu.")
            return False
        kick_choices = []
        for subor_model_class in self.subors:
            kick_choices += self.callback_factories.get(f'hold_{subor_model_class}_idlers', lambda: lambda: {})()()
        for choice in kick_choices:
            self.callback_factories.get(f"move_{choice[0]}_model")()(choice[1], 'cpu')
        #self.revive_holded_idlers([[model_class, model_name]])
        remain_vram = get_remain_vram()
        if remain_vram < target_model_info[model_name]['size']:
            disp.red(f"force_reallocate() made an vram clearance of {remain_vram}, which is still smaller than model size {target_model_info[model_name]['size']}.")
            self.revive_holded_idlers(kick_choices)
            return False
        self.callback_factories.get(f"move_{model_class}_model")()(model_name, 'cuda')
        self.revive_holded_idlers(kick_choices)
        return True
