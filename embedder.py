...
from utils import access_nested_dict, get_model_size, estimate_tokens
...
class Embedder(BasicEmbedder,AbsLiveModel):
    ...
    def embed_as_openai(self, strings: List[str], model_name = None, output_dims: int = -1, tokens_estimation: str = 'len') -> Dict:
        ...
        if self.params.get('show'):
            disp.purple(f'embedded data dimensions: {len(embeds)}, {len(embeds[0])}')
        return retdict
